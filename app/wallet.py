...
# المنصات اللي تدعم المحاولة البديلة المدفوعة (دويين/RedNote/ويشات عبر TikHub)
FALLBACK_PLATFORMS = ("douyin", "rednote", "wechat")

# منصة X: رصيد لتخطي حد الرابط المباشر (تحميل ملفات كبيرة توصل 200 ميكا عبر تليگرام مباشرة
# بدل الرابط المباشر بالمتصفح). آلية استهلاك مختلفة عن الفولباك لكن نفس مبدأ المحفظة.
BIG_FILE_PLATFORMS = ("x",)

# كل المنصات المدفوعة مجتمعة (يُستخدم بمكان واحد فقط: تخطي لمت حجم الملفات العام،
# لأن أي مستخدم عنده رصيد بأي غرض مدفوع يُعتبر "مشترك" ويتخطى ذاك اللمت تلقائياً)
PAID_PLATFORMS = FALLBACK_PLATFORMS + BIG_FILE_PLATFORMS
...
DEFAULT_PACKAGES = {
    ...
    # X: لا تكلفة TikHub هنا (yt-dlp مجاني)، فالتسعير أقل بكثير - هدفه تغطية بندويث/رفع
    # تليگرام لملفات كبيرة (حتى 200 ميكا)، مو تعويض تكلفة API خارجي.
    "x": [
        {"credits": 10, "stars": 10},
        {"credits": 30, "stars": 25},
        {"credits": 100, "stars": 70},
    ],
}
